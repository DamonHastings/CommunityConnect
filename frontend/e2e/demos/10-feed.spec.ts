...
test("Org admin browses the activity feed and clicks through to an item", async ({ page }) => {
  ...
  // ── Activity feed ─────────────────────────────────────────────────────────
  // Navigate and wait for the feed API response before asserting on items
  const feedReady = page.waitForResponse(
    (resp) => resp.url().includes("/api/v1/feed") && resp.status() === 200,
    { timeout: 15000 },
  );
  ...
  // Feed should contain items — match by known seeded content
  const firstFeedItem = page
    .getByRole("link")
    .filter({
      hasText:
        /Food Pantry|Meal Delivery|Youth Mentorship|Summer Job|Weekly Tutoring|Grant|Holiday Food|Summer Program|Corporate Partnership/i,
    })
    .first();
  await expect(firstFeedItem).toBeVisible({ timeout: 10000 });
  await PAUSE(1500);
  ...
  const programLink = page
    .getByRole("heading", { name: /Summer Job Training|Weekly Tutoring|Food Pantry|Tutoring/i })
