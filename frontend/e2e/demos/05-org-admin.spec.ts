...
test("Org admin reviews applications, posts an announcement, and adds an opportunity", async ({
  page,
}) => {
  ...
  const res = await page.request.get(
    "http://localhost:3001/api/v1/organizations?q=City+Food+Bank",
    {
  ...
  const weekendFoodPantryItem = page
    .locator('tr, article, [class*="card"], [class*="row"]')
    .filter({ hasText: "Weekend Food Pantry Volunteer" })
    .first();
  await expect(weekendFoodPantryItem).toBeVisible({ timeout: 10000 });
  await PAUSE(1200);

  await smoothClick(weekendFoodPantryItem.getByRole("button", { name: "View applications" }));
  await PAUSE(1000);
  // Scroll down to show Jason's application row
  const jasonRow = page
    .locator('tr, article, [class*="items"]')
  ...
  await smoothFill(
    page.getByPlaceholder("Announcement title"),
    "Summer Volunteer Drive — Join Us!",
  );
  await PAUSE(600);
